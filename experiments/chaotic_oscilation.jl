...
num_diff(x) = sum(x[x .> 0])
V = spike_to_vec(sp, N)
Vdelta = spike_to_vec(sp_delta, N)
diff = abs.(V .- Vdelta)
shared = V .* Vdelta

err = reshape(mapslices(num_diff, V - Vdelta, dims=1), :)
...
map(x -> x.plasticity = 0.05, brain.areas)
_, sp_plastic, ds_plastic = simulate!(
   [zero_stim(brain[1]), zero_stim(brain[2])],
   [a0],
   max_iters=timesteps,
   tol=0,
...
V_plastic = spike_to_vec(sp_plastic, N);
